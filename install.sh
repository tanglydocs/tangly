...
case "\$RUNNER" in
  bun)
    ...
    ;;
  npm)
    # Install once per version into a stable cache; invoke the bin directly
    # so we don't race the dash/sh PATH-injection bug that npx can hit on
    # some distros (\`sh: tangly: not found\` after npx finishes its
    # download phase).
    CACHE_DIR="\${TANGLY_CACHE:-\${XDG_CACHE_HOME:-\$HOME/.cache}/tangly}/\$TANGLY_VERSION"
    BIN="\$CACHE_DIR/node_modules/.bin/tangly"
    if [ ! -x "\$BIN" ]; then
      mkdir -p "\$CACHE_DIR"
      (cd "\$CACHE_DIR" && npm install --silent --no-save --no-audit --no-fund "tangly@\$TANGLY_VERSION" >/dev/null)
    fi
    exec "\$BIN" "\$@"
    ;;
esac
