...
<CardGroup cols={2}>
  ...
  <Card title="Components" icon="puzzle" href="/reference/components">
    39 MDX components: Card, Tabs, Steps, OpenAPI, callouts, and more.
  </Card>
...
- **[39 MDX components](/reference/components), no imports.** Cards, Tabs, Steps, Accordions, callouts, ParamFields, CodeGroups, and more. Every theme ships every one.
...
- **Six themes, or your own.** `tang`, `pith`, `pip`, `readable`, `geist`, `cirrus` via one `theme` field. Build [custom themes](/guides/themes) from `@tanglydocs/theme-ui`. See [live demos](https://examples.tangly.dev).
