...
<Steps>
  ...
  <Step title="Run migrate">
    ...
    - **If you have `docs.json`** (Mintlify modern): validates against Tangly's schema, updates `$schema` to point at Tangly, swaps Mintlify theme aliases (`mint`, `maple`, `palm`, …) to `tang`. Use `--keep-theme` to skip the theme swap.
...
<ParamField path="--keep-theme" type="boolean">
  Don't swap Mintlify theme names (`mint`, `maple`, etc.) to `tang`. Useful if you want to keep the schema validating against your existing tooling.
</ParamField>
