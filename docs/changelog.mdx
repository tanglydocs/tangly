...
<Update label="v0.5.0" description="13 Sep 26" tags={["feat", "fix"]}>
  ## Highlights

  Every page ships JSON-LD structured data.

  - **One entity, not one per page.** Each page now carries a single `<script type="application/ld+json">` holding an `@graph`: `Organization`, `WebSite`, `WebPage`, `BreadcrumbList`, and a `TechArticle` for pages with a body. The site-level nodes carry the same `@id` on every page and are *referenced* by the page-level nodes rather than restated, so a crawler reconciles a sixty-page site to one publisher instead of sixty. ([#18](https://github.com/tanglydocs/tangly/issues/18))
  - **Nothing to configure.** An existing project gets a correct graph from its `name`, `siteUrl` and `logo`. New optional fields: root `sameAs` for your other homes (social profiles, GitHub, the marketing site), and `seo.organization.{name,url,logo,sameAs}`, `seo.jsonld`, `seo.locale`. Per page: `jsonld`, `schemaType`, `author`, `datePublished`, `dateModified`. Every one is optional and additive, so a `docs.json` copied from Mintlify parses unchanged.
  - **Dates come from git, never the build clock.** `dateModified` is the last commit touching the page's source, `datePublished` the commit that added it, both read in one `git log` pass per build. A nightly rebuild no longer claims every page changed last night. Outside a git repository the dates are simply omitted.
  - **Emitted once, in the shared head fragment**, so all six themes get it without changes. `tangly dev` pretty-prints the graph so you can read it in view-source; `tangly build` minifies it.

  ## Changes

  ### Features
  - feat(schema): `buildJsonLd` graph builder, root `sameAs`, `seo.{jsonld,locale,organization}`, and the five new frontmatter fields ([#18](https://github.com/tanglydocs/tangly/issues/18))
  - feat(runtime): emit the JSON-LD graph from the shared `<Seo>` head fragment, and the site-level nodes from the redirect stub served at `/`
  - feat(manifest): resolve the nav trail, the first-commit date and a body check per page at build time
  - feat(theme-ui): `?q=` opens the search modal with the query already run, which is what the `SearchAction` target points at
  - feat(theme-ui): `seo.locale` drives `<html lang>` as well as `inLanguage`, so the two cannot disagree

  ### Fixes
  - fix(schema): accept an unquoted YAML date in `lastUpdated`, `datePublished` and `dateModified`. YAML parses a bare `2026-01-02` into a `Date`, which the string-only schema rejected, failing the page's whole frontmatter rather than the one field
  - fix(manifest): resolve symlinks before keying git metadata by path. A project reached through a symlink (macOS `/tmp`, a symlinked checkout) silently lost every last-updated date

  ### Tests
  - test(schema): 35 cases over the graph builder, covering defaults, overrides, opt-out, breadcrumbs, dates present and absent, a missing logo, and the `</script` escape
  - test(build): both tarball smokes assert one parseable ld+json block per built page with absolute `@id`s; CI validates the built docs site end to end
</Update>
...
<Update label="v0.3.0" description="25 Aug 26" tags={["feat"]}>
  ...
  A sixth theme, a quieter code block, and a component for handing setup instructions to a coding agent.

  - **Cirrus, the infrastructure-docs theme.** `{ "theme": "cirrus" }` gets you the Cloudflare/Fastly register: full-bleed shell with a wide centred measure, dense type, hairline rules under headings, one warm accent. Built for reference material you scan rather than prose you read start to finish. Links derive from `--tangly-color-primary` with `color-mix` instead of restating it, so setting `colors.primary` recolors links to match rather than leaving them orange. ([#15](https://github.com/tanglydocs/tangly/pull/15))
  - **Bare code blocks.** `styling.codeblocks.chrome: "bare"` drops the caption bar and floats the Copy button over the top-right of the block, which reads better for one-liners where the header was taller than the code it labelled. Pair it with `copyButton: "hover"` to reveal the button on hover or keyboard focus; touch devices always see it. A single fence overrides the site default in either direction with a `bare` or `bar` meta flag. ([#15](https://github.com/tanglydocs/tangly/pull/15))
  - **`<CopyPrompt />`.** A button that copies one line pointing a coding agent at a URL of setup instructions, rather than pasting the instructions at the reader. The instructions then live at that URL and can change without anyone re-copying. A relative `href` picks up the configured base at build and resolves against the current document in the browser at click time, because the agent receiving the prompt has no page context. ([#15](https://github.com/tanglydocs/tangly/pull/15))

  ## Changes

  ### Features
  - feat(theme-cirrus): add cirrus theme, a dense infra-docs aesthetic, with a full-bleed shell and a wide centred measure ([#15](https://github.com/tanglydocs/tangly/pull/15))
  - feat(runtime): `styling.codeblocks.chrome` (`"bar"` | `"bare"`) plus per-fence `bare` / `bar` meta flags ([#15](https://github.com/tanglydocs/tangly/pull/15))
  - feat(theme-ui): `<CopyPrompt />` component for agent onboarding ([#15](https://github.com/tanglydocs/tangly/pull/15))

  ### Fixes
  - fix(schema): `styling.codeblocks.copyButton` was read by the runtime but missing from the strict schema object, so setting it failed validation
  - fix(runtime): `astro.config.mjs` reads code-block settings from `styling.codeblocks`, the schema'd Mintlify-compatible location, not only the older top-level `code` key. A valid `styling.codeblocks` used to be silently ignored
  - fix(theme-cirrus): align nav, tabs and sidebar labels to one left edge
  - fix(theme-ui): the solid navbar CTA takes its text colour from `--tangly-color-primary-fg` (default white), so a theme with a light accent can clear WCAG AA instead of being stuck with hardcoded white

  ### Internal
  - build(release): register `@tanglydocs/theme-cirrus` in the release planner. The `PUBLISHABLE` list is hand-maintained, so a new package was invisible to it and a release would have published `tangly` depending on a version that was never pushed to npm

  ### Docs
  - docs: `/guides/themes/cirrus`, the `styling.codeblocks` object form in the schema reference, and `<CopyPrompt />` plus a code-block chrome section in the components reference
</Update>
...
<Update label="v0.2.3" description="17 Jul 26" tags={["fix"]}>
  ...
  SEO fix for the home page of every built site.

  - **Published URLs point at the served route, not `/index`.** A root `index.mdx` renders at `/` but is also routable at `/index`, which redirects. The sitemap, `llms.txt`, and the page-markdown outputs were emitting `/index`, so crawlers saw a redirecting duplicate of the home page. All three now emit `/`, matching the canonical tag. Nested `foo/index.mdx` pages were never affected (they collapse to `foo` during the page scan). ([`5a7ae97`](https://github.com/tanglydocs/tangly/commit/5a7ae97cf3605eb6453edd42ddbe8f82a095b5de))

  ## Changes

  ### Fixes
  - fix(build): sitemap, `llms.txt`, and page-markdown emit the served route for index pages instead of `/index` ([`5a7ae97`](https://github.com/tanglydocs/tangly/commit/5a7ae97cf3605eb6453edd42ddbe8f82a095b5de))
  - fix(theme-ui): `Seo.astro` resolves the canonical URL through the shared route helper, so canonical tags and sitemap entries can no longer diverge ([`5a7ae97`](https://github.com/tanglydocs/tangly/commit/5a7ae97cf3605eb6453edd42ddbe8f82a095b5de))

  ### Internal
  - refactor(schema): add `@tanglydocs/schema/page-path` (`pageRouteForSlug` / `pagePathForSlug`) as the single slug-to-route mapping, shared by the CLI emitters and the themes without importing `tangly`

  ### Tests
  - test(schema): cover root, nested, and trailing-slash index slugs in `page-path.test.ts`; assert the sitemap emits `/` for the home page
</Update>
