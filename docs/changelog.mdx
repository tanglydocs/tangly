...
<Update label="v0.1.1" description="2026-06-03" tags={["fix"]}>
  ## Highlights

  A theme polish pass on the reading experience.

  - **Cleaner code blocks.** The code body now has a single border with square top and rounded bottom corners, instead of doubled borders and all-four rounded corners. Code-group tabs switched to readable underline tabs, and the Copy button got a clipboard icon (a check on success).
  - **Sidebar keeps your place.** The nav no longer resets to the top on every navigation. It restores scroll position and keeps the active page in view, and gained bottom padding so the last item is not clipped under the announcement banner.
...
<Update label="v0.1.0" description="2026-05-12" tags={["docs","feat","fix","refactor"]}>
  ## Highlights
