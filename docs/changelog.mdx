...
<Update label="v0.2.0" description="8 Jun 26" tags={["feat","fix","test"]}>
  ...
  - **Reusable snippet imports + a `Link` component.** `import` of an `/snippets/*` Markdown file now resolves against your project root, and `Link` (with `href`) is a built-in. Broader Font Awesome icon coverage rounds it out.
