...
flowchart TD
  ...
  subgraph cli[tangly CLI]
    B1["@tangly/schema<br/>parse + validate"]
    B2[manifest builder]
...
<CardGroup cols={3}>
  ...
  <Card title="@tangly/theme-ui" icon="palette">
...
<Note>
  **Component shadowing** resolves bottom-up: project `theme/<Name>.astro` → active theme → built-in default. The Astro integration sets up Vite aliases so `@tangly/theme-ui/Card.astro` transparently resolves to whatever the user has overridden.
</Note>
