...
`_astro/` is reserved for Astro's hashed asset bundles. Putting a file under `_astro/` is a hard error — the build refuses to overlay it. If you really need to ship a file under that path (you don't), pick a different prefix.
