...
Default themes are `vitesse-light` / `vitesse-dark`, switching automatically with the user's color preference. Any [bundled Shiki theme](https://shiki.style/themes) works. Override per-project:
...
Twoslash (rich TypeScript hovers in code blocks) is on the roadmap; not wired in v0.2. Track [Phase 6](/guides/customization/components) for status.
...
- [Configuration → Code](/guides/customization/configuration#code-blocks): the `docs.json` knobs.
