...
See [API auth](/reference/schema#auth-methods) for headers.
```
...
Every image gets a lightbox on click. To opt out, see [`<LightboxImage>`](/reference/components/media).
...
The slug is what you reference in [`docs.json#navigation`](/reference/schema) and what you link from other pages.
