---
title: Deploying
description: Ship your docs to Vercel, Cloudflare, Netlify, Docker, or any static host.
icon: "rocket"
---
...
Workers with [static assets](https://developers.cloudflare.com/workers/static-assets/) is Cloudflare's current way to host a static site (Pages is in maintenance mode). An assets-only Worker needs no script — just a `wrangler.jsonc`:
...
- **`html_handling: "drop-trailing-slash"`** — Tangly emits directory-style output (`cli/audit/index.html`) but canonical URLs without a trailing slash (`/cli/audit`, Mintlify parity). Cloudflare's default (`auto-trailing-slash`) 307-redirects every page to the slash variant — each URL then contradicts its own canonical. `drop-trailing-slash` serves `/cli/audit` directly and redirects `/cli/audit/` down to it, matching the canonicals.
- **`not_found_handling: "404-page"`** — serves Tangly's `dist/404.html` (your `404.mdx` if you have one) for unknown routes.
...
For a git-connected Pages project, in the Cloudflare dashboard:
...
<Note>
  Cloudflare serves the static `dist/` regardless of the `cloudflare` adapter. The adapter signal is preserved for when SSR routes land and Workers becomes the runtime.
</Note>
...
For same-URL content negotiation (`Accept: text/markdown` returns Markdown, browsers still get HTML), add a one-line CDN rewrite. See [Markdown for agents](/guides/ai-agents/markdown-for-agents) for Vercel, Cloudflare, and Netlify recipes.
