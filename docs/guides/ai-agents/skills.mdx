...
This drops the skill files into your global skills directory (`~/.claude/skills/` for Claude Code's CLI). Drop the `-g` to install into the current project's skills dir instead. Claude Code picks them up at session start.
