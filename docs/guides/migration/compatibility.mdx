...
| `navigation` | ⚠️ | Tabs, groups, anchors, dropdowns render with identical recursion. `versions` / `languages` parse (schema-accepted) but aren't rendered yet: their groups are flattened into one sidebar, so a migrated multi-version project shows every version's groups at once instead of a switcher. |
