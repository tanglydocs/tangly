...
`import { X } from "/snippets/foo.mdx"` resolves to `<project>/snippets/foo.mdx`. Markdown/MDX snippets render. Two caveats: a snippet that itself uses MDX components doesn't yet inherit the global component set (those components render undefined), and `.jsx`/`.tsx` component snippets resolve but need a UI-framework integration to render (see [What's not in Tangly yet](#whats-not-in-tangly-yet)).
...
- **Snippet component injection.** An imported MDX snippet that uses MDX components doesn't yet inherit the global component set, so those components render undefined.
- **JSX/React snippet components.** `import { X } from "/snippets/X.jsx"` resolves, but Tangly renders MDX + its built-in components, not arbitrary user React islands. Use an MDX snippet, [`<Embed>`](/reference/components/media), or a `<Video>`/`<iframe>` instead.
