...
<Note>
  Wrapping your template in `<Layout>` (as above) gives it canonical tags, Open Graph, and social cards for free. If you render your own `<head>` instead, import `Seo` to keep them: `import Seo from "@tanglydocs/theme-ui/Seo.astro";` then `<Seo config={manifest.config} page={page} pageTitle={title} pageDescription={description} />`.
</Note>
