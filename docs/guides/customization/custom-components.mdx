...
Tangly's auto-injected component map covers the 29 components listed above. To use a component that isn't in that set (e.g. a custom `<Tldr>`), import it explicitly in your MDX:
...
For visual-only changes (colors, spacing, typography), you don't need to shadow the component. Drop a `theme/styles/theme.css` and override CSS variables. See [Themes](/reference/schema#themes).
