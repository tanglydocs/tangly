...
Tangly pages are MDX: Markdown with embedded JSX. The Markdown half is GitHub-flavored CommonMark plus a few additions. The JSX half lets you drop a [component](/reference/components) anywhere a paragraph would go.
...
The page H1 is rendered automatically from the `title` frontmatter field. **Don't write your own `# Title`** at the top, or you'll get two H1s.
...
For the same code in multiple languages, wrap in [`<CodeGroup>`](/reference/components/code#codegroup). It auto-syncs across pages on known label families (npm/yarn/pnpm/bun, macOS/Linux/Windows, etc.).
...
Tangly renders [Mermaid](https://mermaid.js.org) diagrams from fenced ```` ```mermaid ```` blocks. Supports flowcharts, sequence, class, state, ER, gantt, pie, journey, mindmap, timeline, and the rest of Mermaid's families.
...
Mermaid uses the `base` theme with `themeVariables` driven by Tangly's brand tokens. Diagrams pick up the primary tang accent and surface/ink colors from the active theme. Toggling light/dark re-renders every diagram on the page (`tangly:rerender-mermaid` event).

### Performance

The Mermaid library (~600 KB) is dynamically imported **only on pages that contain at least one `pre.mermaid` element**. Pages without diagrams pay nothing.

### Security

Mermaid runs in `securityLevel: "strict"`. HTML and `<script>` inside diagram source are sanitized.
...
`<Video>` detects the provider from `src` and rewrites to the privacy-mode embed where applicable. Lazy-loaded, with no third-party traffic until the player scrolls into view.
