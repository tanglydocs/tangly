...
`docs.json` is the source of truth for everything that isn't page content: branding, navigation, integrations, behavior. The full schema is defined in [`packages/schema/src/docs-json.ts`](https://github.com/tanglydocs/tangly/blob/main/packages/schema/src/docs-json.ts).
...
The nav tree. Three forms; pick one or combine. Schema in [`packages/schema/src/navigation.ts`](https://github.com/tanglydocs/tangly/blob/main/packages/schema/src/navigation.ts).
...
| `theme` | `string \| { light, dark }` | Shiki theme. Default: `vitesse-light` / `vitesse-dark`. |
...
| `codeblocks` | `"system" \| "dark"` \| object | Force code blocks to always use dark theme regardless of page mode, or pass an object for finer control |

The object form takes three keys:

| Field | Type | Default | Description |
|---|---|---|---|
| `theme` | `string \| { light, dark }` | `vitesse-light` / `vitesse-dark` | Shiki theme name, or a light/dark pair |
| `chrome` | `"bar" \| "bare"` | `"bar"` | `bare` drops the caption bar and floats the Copy button over the code. A fence overrides it with a `bare` or `bar` meta flag |
| `copyButton` | `boolean \| "always" \| "hover"` | `true` | `false` removes the button; `"hover"` (bare chrome only) reveals it on hover or keyboard focus |
...
See [code block chrome](/reference/components/code#code-block-chrome) for what each variant looks like.
...
Controls auto-generated social cards (the Open Graph / Twitter preview image shown when a page is shared). Tangly renders a branded 1200×630 card per page from the page title, description, the owning tab, your theme, and your `colors` / `logo`. Cards generate by default once [`siteUrl`](#siteurl) is set (an absolute URL is required for `og:image`).

| Field | Type | Description |
|---|---|---|
| `enabled` | `boolean` | Master switch. Default `true`. Set `false` to turn off generation. |
| `background` | `string` | Card background: a hex color (e.g. `"#0B0B0F"`). Defaults to the theme surface. |
| `accent` | `string` | Accent color (hex). Defaults to `colors.primary`. |
| `image` | `string` | One static image used as `og:image` for every page, instead of generated cards. |

```json
{ "thumbnails": { "accent": "#EA580C" } }
```

Cards are prerendered to `/og/<slug>.png` at build (and rendered on demand in `tangly dev` for live preview). Per-page overrides: set [`seo.ogImage`](/reference/schema/frontmatter#seo) in a page's frontmatter. Drafts and `noindex` pages get no card.
