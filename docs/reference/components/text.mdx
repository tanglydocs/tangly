...
3. Font Awesome / Mintlify alias map (`magnifying-glass` → `Search`, `gear` → `Settings`, `xmark` → `X`, `angle-down` → `ChevronDown`, …)
4. PascalCase fallback (`arrow-right` → `ArrowRight`, `my-icon` → `MyIcon`)
5. Fallback to `CircleHelp` (with a console warning)

Together, the alias map and the PascalCase fallback cover roughly **880 Font Awesome names** — Font Awesome 6 renames, directional families (`angle-*`/`caret-*` → `Chevron*`), and nearest-glyph picks — so most Mintlify icon names render unchanged after migrating. Names with no Lucide equivalent (brand-specific glyphs, digits, animals, niche objects) fall back to the help-circle. The alias map lives in [`fa-to-lucide.ts`](https://github.com/tanglydocs/tangly/blob/main/packages/theme-ui/src/components/fa-to-lucide.ts).
...
<GlossaryTerm
  ...
  term="Capacity factor"
  definition="The ratio of actual energy generated to the theoretical maximum if a plant ran at full power continuously."
  href="/guides/authoring/glossary"
>
  CF
</GlossaryTerm>
